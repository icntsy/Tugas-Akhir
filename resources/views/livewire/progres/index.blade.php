                    ...
                    <div class="col-md-6 col-sm-12 d-flex justify-content-end">
                        <div class="input-group">
                            <input type="text" class="form-control form-control" wire:model.lazy="search"
                                placeholder="{{ __('Cari Data Pemeriksaan') }}" value="{{ request('search') }}">
                        ...
                        <table class="mb-0 table table-striped table-bordered">
                            ...
                            <tbody>
                                @forelse($queues as $index => $queue)
                                    <livewire:progres.single :queue="$queue" :progresIndex="$index + $queues->firstItem()" :key="$queue->id" />
                                @empty
                                {{-- @forelse($queues as $queue)
                                    <livewire:progres.single :queue="$queue" :key="time() . $queue->id" />
                                @empty --}}
